...
test("deal refuses below the minimum player count", () => {
  throwsWith("NOT_ENOUGH_PLAYERS", () => dealRound(["a", "b"], GRID, seeded(1)));
});

test("deal refuses a grid that is not exactly 16", () => {
  throwsWith("INTERNAL", () =>
    dealRound(P, { ...GRID, cells: GRID.cells.slice(0, 15) }, seeded(1)),
  );
});

test("cell positions move between rounds, so a grid cannot be learnt by layout", () => {
  const a = dealRound(P, GRID, seeded(3)).cells;
  const b = dealRound(P, GRID, seeded(99)).cells;
...
test("clues are rejected outside the clue phase, twice, or from outsiders", () => {
  ...
  const voting = callVote({ ...s, clues: { a: "1", b: "2", c: "3", d: "4" }, phase: "discuss" }, "a");
...
test("accusing an innocent ends the round and the Girgit escapes", () => {
  ...
  let s = dealRound(P, GRID, fixed(0));
...
function toEscape(): RoundState {
  let s = dealRound(P, GRID, fixed(0)); // girgit = a, secret = cell index 0
